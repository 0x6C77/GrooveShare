...
Channel.prototype.checkConcerts = function(artist) {
    if (!config.get('songkick')) return;

    http.get('http://api.songkick.com/api/3.0/events.json?apikey='+config.get('songkick')+'&artist_name='+encodeURIComponent(artist)+'&location=sk:24426', function(res) {
        var body = '';
