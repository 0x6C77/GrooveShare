...
Library.prototype.getRandomTrackID = function(callback) {
    ...
    this.db.all("SELECT id, julianday('now') - julianday(ifnull(last,added)) AS weight FROM tracks ORDER BY weight", function(err, tracks) {
...
Library.prototype.lookupTrackID = function(id, callback) {
    ...
    if (callback && typeof(callback) === "function") {
        var stmt = this.db.prepare("SELECT * FROM tracks WHERE id = ?");
