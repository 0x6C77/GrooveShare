 ...
 * @param {Number} uuid
 */
Library.prototype.rateTrack = function(trackID, uuid, rating) {
    ...
    try {
        ...
        stmt.run(uuid, trackID, function() {
            // Add new rating
            var stmt = self.db.prepare("INSERT INTO tracks_ratings (`uuid`, `track`, `rating`) VALUES (?, ?, ?)");
            stmt.run(uuid, trackID, rating);

            // Get updated ratings
            self.lookupTrackID(trackID, function(track) {
                self.emit('rated', {trackID: trackID, up: track.up, down: track.down, up_uuid: track.up_uuid, down_uuid: track.down_uuid});
            })
        });
    } catch(e) {
        console.log(e);
...
Library.prototype.addTrack = function(data) {
    ...
    try {
        ...
        console.log(data.id, data.track, data.artist, data.image, data.YTID);
...
Library.prototype.getRandomTrackID = function(channel_id, callback) {
    ...
    var stmt = this.db.prepare("SELECT\
                    id,\
                    julianday('now') - julianday(IFNULL(channels_tracks.last, channels_tracks.added)) - (IFNULL(ratings_down.down, 0)*2) + IFNULL(ratings_down.down, 0) AS weight\
                 FROM tracks\
                 INNER JOIN channels_tracks\
                 ON channels_tracks.channel_id = ? AND channels_tracks.track_id = tracks.id\
