 ...
 */
TrackManager.prototype.findSong = function(q, channel, callback) {
        ...
        if (response.trackmatches.track) {
            ...
            for (i = 0; i < resultsLength; i++) {
                ...
                if (item.mbid) {
                    ...
                    tmpItem.added = Boolean(global.library.lookupTrackID(tmpItem.id, channel));
                } else {
                    // Generate an ID
                    ...
                    tmpItem.added = Boolean(global.library.lookupTrack(item.track, item.artist, channel));
                }

                // TODO: correct channel added checks
                tmpItem.added = false;
 ...
 */
TrackManager.prototype.addSong = function(q, channel, callback) {
    if (!q) return;
    ...
    lastFM.track.getInfo(opts, function (err, response) {
        ...
        if (global.library.lookupTrackID(q, channel)) {
            // Just say it was done
            callback(data);
            return;
        }
...
TrackManager.prototype.checkVideoResults = function(results, data, callback, n) {
    ...
    if (n >= results.length || n > 10) {
        if (n < 99) {
            // Screw it lets try the first one
            data.duration = null;
            console.log('Removing duration');
            this.checkVideoResults(results, data, callback);
        } else {
    ...
    ytdl.getInfo('http://www.youtube.com/watch?v=' + result.id, {downloadURL: true}, function(err, info) {
        ...
        if (duration && (info.length_seconds < duration * 0.8 || info.length_seconds > duration * 1.5 || info.length_seconds > 600)) {
            ...
            self.checkVideoResults(results, data, callback, 999);
            return;
        }
